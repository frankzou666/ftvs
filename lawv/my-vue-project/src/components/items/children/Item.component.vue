...
export default defineComponent({
    ...
    setup(props,{emit}) {
        ...
        const handelClick=()=>{ emit("selectItem",props.model.id);};


        return {cssClass,handelClick};
...
export default defineComponent({
 ...
 // add testid prop here as well 
  ...
  model: {
    type: Object as PropType<ItemInterface>, default: () => {
    return {} }
  } },
  ...
  setup(props, { emit }) {
   const cssClass = computed(() => {
      let css = 'item'
      if (props.model.selected) { css += ' selected'}
      return css.trim() })
   const handleClick = () => {
